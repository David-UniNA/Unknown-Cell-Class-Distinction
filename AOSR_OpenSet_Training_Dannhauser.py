...
basic_model = tf.keras.models.load_model(test_DIR + '\\CNN__AOSR_100_class4_1.keras') # Load the pre-trained closed-set model from the specified directory.
dim = 100                                                                   # Set the image dimension for resizing images. (should be the same of the pre-trained closed-set training)
beta_values = np.linspace(0.0, 1, 6)                                        # Generate an array of beta values from 0.0 to 0.5
beta_show = 0.05                                                            # Beta value used for oresented AOSR prediction
Epo1 = 40                                                                   # Epoch number of 1st AOSR prediction round
Epo2 = 200                                                                  # Epoch number of 2nd AOSR prediction round
Bat = 32                                                                    # Batch size (defualt = 32)
LABELunknown = 4                                                            # Label number of Unknowns
LABELtotal = 5                                                              # Total number of labels (unknonws included)
IF = 1000                                                                   # Sample size of Isolation Forest
seed = 0                                                                    # Random seed value (default = 0)
...
## Import unknown samples (without Load_Dataset.py)
THP = glob.glob(input_folder + '\\DataSET\\train\\THP\\*.tif')              # Retrieve all .tif files from the specified path for unknown samples (THP).
...
## Feature Encoding
tf.random.set_seed(seed)                                                    # Set a random seed for reproducibility in TensorFlow operations.
...
Encoder = tf.keras.models.Model(inputs=basic_model.layers[0].input,         # Accesses the first layer of the model
                                outputs=basic_model.layers[-2].output)      # Accesses the second-to-last layer in the model (since negative indexing counts from the end of the list)

## Use the Encoder model to generate encoded feature vectors for the training dataset (X_INI) and the unknown dataset (X_THP).
Encoder_INI = Encoder.predict(X_INI)                                        # Encoded features from the training dataset.
Encoder_THP = Encoder.predict(X_THP)                                        # Encoded features from the unknown dataset.
...
AOSR = tf.keras.models.Sequential([
       tf.keras.layers.Dense(LABELtotal),                                   # Output layer with 5 units (for 5 classes)
...
optimizer = tf.keras.optimizers.Adam(learning_rate=1e-3, clipnorm=1.0)      # Clipnorm apply gradient clipping based on the norm of the gradients. This helps to prevent the gradients from becoming too large during the training of a neural network, which can lead to instability and convergence issues.
...
AOSR.fit(Encoder_INI, Y_INI, epochs=Epo1)                                   # 1st phase of AOSR training

sample_test, _ = sample_enrichment_IF(seed, Encoder_INI, IF)                # Sample a subset (1000) of the encoded feature vectors (Encoder_X) for testing.
...
for beta in beta_values:
    ...
    fit_params = {'epochs': Epo2, 'batch_size': Bat}                        # Set training parameters
